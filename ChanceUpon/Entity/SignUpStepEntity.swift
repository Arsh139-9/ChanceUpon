//
//  SignUpStepEntity.swift
//  ChanceUpon
//
//  Created by Dharmani Apps mini on 11/25/20.
//  Copyright © 2020 Apple. All rights reserved.
//

import Foundation

struct SignUpStepData<T>{
  
    var isProfileComplete:String
    var alertMessage:String
    var status:String
    var userDetailEntity:SignUpStepUserDetail<T>
    
    init?(dict:[String:T]) {
        let status  = dict["status"] as? String ?? ""
        let isProfileComplete  = dict["isProfileComplete"] as? String ?? ""

        let alertMessage = dict["message"] as? String ?? ""
       
        let userDetailDict = dict["userDetails"] as? [String:T] ?? [:]
       
        let userDetailTStruct = SignUpStepUserDetail(dict: userDetailDict)!
